...
export default function Write({ isWriter, students, setIsModal ,isPatch, data}){
  ...
  useEffect(() => {
    const getPlace = async () => {
      const newPlace = await searchPlace(location.state.place);
      return newPlace[0]
    }
    getPlace().then((place) => {
      setPlace(place);
  ...
  if(isPatch){
    ...
    <S.WriteContainer>
        ...
        <S.Wrap>
          ...
          <S.Submit>
            <SquareBtn name={"돌아가기"} status={false} On={() => navigate(-1)} />
            <SquareBtn
              name={"작성완료"}
              status={true}
              On={() => {
                postMovement({ selectStudentShow, selectOrganization , day, time : location.state.period, place : place, cause, recordDay,teacher_id : id, teacher_name : name, selectStudent })
                setDayComponent(recordDay);
              }}
            />
          </S.Submit>
